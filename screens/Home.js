...
const Home = ({ navigation }) => {
    const { books, bookmarks, booksBestSeller, booksTheLatest, booksComingSoon } = useSelector(state => state.booksReducer);
    const dispatch = useDispatch();
  
    const fetchBooks = () => dispatch(getBooks());
    const fetchBooks1 = () => dispatch(getBookBestSeller());
    const fetchBooks2 = () => dispatch(getBookTheLatest());
    const fetchBooks3 = () => dispatch(getBookComingSoon());

    const b = books.filter(books => books.status == 'Best Seller')
